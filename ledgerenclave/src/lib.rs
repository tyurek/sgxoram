...
pub use mc_fog_ledger_enclave_api::{
    EnclaveCall, Error, LedgerEnclave, LedgerEnclaveProxy, Result,
};
...
impl LedgerSgxEnclave {
    ...
    pub fn new(
        ...
        sgx_enclave
            .enclave_init(desired_capacity)
            .unwrap_or_else(|e| panic!("enclave_init failed: {:?}", e));
...
impl LedgerEnclave for LedgerSgxEnclave {
    ...
    fn add_omap_item(&self, key: Vec<u8>, value: Vec<u8>) -> Result<()>{
        let inbuf = mc_util_serial::serialize(&EnclaveCall::AddOmapItem(key, value))?;
        let outbuf = self.enclave_call(&inbuf)?;
        mc_util_serial::deserialize(&outbuf[..])?
    }
    fn get_omap_item(&self, key: Vec<u8>) -> Result<Vec<u8>>{
        let inbuf = mc_util_serial::serialize(&EnclaveCall::GetOmapItem(key))?;
        let outbuf = self.enclave_call(&inbuf)?;
        mc_util_serial::deserialize(&outbuf[..])?
    }
    fn add_oram_item(&self, key: u64, value: Vec<u8>) -> Result<()>{
        let inbuf = mc_util_serial::serialize(&EnclaveCall::AddOramItem(key, value))?;
        let outbuf = self.enclave_call(&inbuf)?;
        mc_util_serial::deserialize(&outbuf[..])?
    }
    fn get_oram_item(&self, key: u64) -> Result<Vec<u8>>{
        let inbuf = mc_util_serial::serialize(&EnclaveCall::GetOramItem(key))?;
        let outbuf = self.enclave_call(&inbuf)?;
        mc_util_serial::deserialize(&outbuf[..])?
    }
}
