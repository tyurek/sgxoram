...
pub fn ecall_dispatcher(inbuf: &[u8]) -> Result<Vec<u8>, sgx_status_t> {
    ...
    let outdata = match call_details {
        // Utility methods
        EnclaveCall::EnclaveInit(desired_capacity) => {
            serialize(&ENCLAVE.enclave_init(desired_capacity))
        ...
        EnclaveCall::AddKeyImageData(records) => serialize(&ENCLAVE.add_key_image_data(records))
            .or(Err(sgx_status_t::SGX_ERROR_UNEXPECTED))?,*/
        EnclaveCall::AddOmapItem(key, value) => serialize(&ENCLAVE.add_omap_item(key, value))
            .or(Err(sgx_status_t::SGX_ERROR_UNEXPECTED))?,
        EnclaveCall::GetOmapItem(key) => serialize(&ENCLAVE.get_omap_item(key))
            .or(Err(sgx_status_t::SGX_ERROR_UNEXPECTED))?,
        EnclaveCall::AddOramItem(key, value) => serialize(&ENCLAVE.add_oram_item(key, value))
            .or(Err(sgx_status_t::SGX_ERROR_UNEXPECTED))?,
        EnclaveCall::GetOramItem(key) => serialize(&ENCLAVE.get_oram_item(key))
            .or(Err(sgx_status_t::SGX_ERROR_UNEXPECTED))?,
    };
