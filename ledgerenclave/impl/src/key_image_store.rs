...
use mc_oblivious_traits::{
    OMapCreator, ORAMStorageCreator, ORAMCreator, ObliviousHashMap, OMAP_FOUND, OMAP_INVALID_KEY,
...
type ObliviousRAMCreator<OSC> = PathORAM4096Z4Creator<McRng, OSC>;
...
impl<OSC: ORAMStorageCreator<StorageDataSize, StorageMetaSize>> KeyImageStore<OSC> {
    ...
    pub fn add_item(&mut self, key: Vec<u8>, value: Vec<u8>)-> Result<(), AddRecordsError>{
        let mut cloned_key = A8Bytes::<KeySize>::default();
        let mut cloned_value = A8Bytes::<ValueSize>::default();
        cloned_key.clone_from_slice(key.as_ref());
        cloned_value.clone_from_slice(value.as_ref());
      
        let omap_result_code = self.omap.vartime_write(&cloned_key, &cloned_value, Choice::from(1));
    ...
    pub fn get_item(&mut self, key: Vec<u8>) -> Result<Vec<u8>, AddRecordsError> {
        let mut cloned_key = A8Bytes::<KeySize>::default();
        cloned_key.clone_from_slice(key.as_ref());
        let mut value = A8Bytes::<ValueSize>::default();
        ...
        // Other -> debug_assert!(false)
        {
            let omap_result_code = self.omap.read(&cloned_key, &mut value);
            if omap_result_code == OMAP_INVALID_KEY {
                return Err(AddRecordsError::KeyRejected);
            } else if omap_result_code == OMAP_OVERFLOW {
                return Err(AddRecordsError::MapOverflow(
        ...
        // Copy the data in value[0..8] to result.spent_at which represents the
        // block_index Copy the data in value[8..16] to result.timestamp
        let result: Vec<u8> = value.to_vec();

        Ok(result)
    }
}
    
pub struct OramStore<OSC: ORAMStorageCreator<StorageDataSize, StorageMetaSize>> {
    /// Oblivious map to hold KeyImageStoreRecords
    //KeySize instead of ValueSize?
    oram: Box<<ObliviousRAMCreator<OSC> as ORAMCreator<BlockSize, McRng>>::Output>,

    /// The logger object
    logger: Logger,
}

impl<OSC: ORAMStorageCreator<StorageDataSize, StorageMetaSize>> OramStore<OSC> {
    pub fn new(desired_capacity: u64, logger: Logger) -> Self {
        Self {
            oram: Box::new(<ObliviousRAMCreator<OSC> as ORAMCreator<
                BlockSize,
                McRng,
            >>::create(
                desired_capacity, STASH_SIZE, &mut McRng::default
            )),
            logger,
        }
    }
    
    pub fn add_item(&mut self, key: u64, value: Vec<u8>)-> Result<(), AddRecordsError>{
        let mut cloned_value = A64Bytes::<BlockSize>::default();
        cloned_value.clone_from_slice(value.as_ref());
        let result = self.oram.write(key, &cloned_value);
        Ok(())
    }
    
    
    pub fn get_item(&mut self, key: u64) -> Vec<u8> {
        let value = self.oram.read(key);
        value.to_vec()
    }
}
