...
use aligned_cmov::typenum::{U1024, U2, U2048, U32, U4, U4096, U8192, U64, U16384, U32768, U65536};
...
/// Creator for PathORAM based on 4096-sized blocks of storage and bucket size
/// (Z) of 4, and a basic recursive position map implementation
/*pub struct PathORAM8192Z4Creator<R, SC>
where
    R: RngCore + CryptoRng + 'static,
    SC: ORAMStorageCreator<U8192, U64>,
{
    _rng: PhantomData<fn() -> R>,
    _sc: PhantomData<fn() -> SC>,
}

impl<R, SC> ORAMCreator<U2048, R> for PathORAM8192Z4Creator<R, SC>
where
    R: RngCore + CryptoRng + Send + Sync + 'static,
    SC: ORAMStorageCreator<U8192, U64>,
{
    type Output = PathORAM<U2048, U4, SC::Output, R>;
...
#[macro_export]
macro_rules! create_oram {
    ($cname: ident, $ubigger:ty, $usmaller:ty ) => {
        pub struct $cname<R, SC>
        where
            R: RngCore + CryptoRng + 'static,
            SC: ORAMStorageCreator<$ubigger, U64>,
        {
            _rng: PhantomData<fn() -> R>,
            _sc: PhantomData<fn() -> SC>,
        }

        impl<R, SC> ORAMCreator<$usmaller, R> for $cname<R, SC>
        where
            R: RngCore + CryptoRng + Send + Sync + 'static,
            SC: ORAMStorageCreator<$ubigger, U64>,
        {
            type Output = PathORAM<$usmaller, U4, SC::Output, R>;
            ...
            fn create<M: 'static + FnMut() -> R>(
                ...
                PathORAM::new::<U32PositionMapCreator<$usmaller, R, Self>, SC, M>(size, stash_size, rng_maker)
            }
        }
    };
}

create_oram!(PathORAM8192Z4Creator, U8192, U2048);
create_oram!(PathORAM16384Z4Creator, U16384, U4096);
